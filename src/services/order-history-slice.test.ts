...
describe('Order History Slice', () => {
    ...
    it('should set isConnected to true on wsConnectionSuccess', () => {
        const state = orderHistorySlice(initialState, wsConnectionSuccess());
    ...
    it('should set error on wsConnectionError and mark isConnected as false', () => {
        ...
        const state = orderHistorySlice(initialState, wsConnectionError(errorMessage));
    ...
    it('should set isConnected to false on wsConnectionClosed', () => {
        ...
        const state = orderHistorySlice(connectedState, wsConnectionClosed());
        expect(state.isConnected).toBe(false);
    });
    ...
    it('should set orders, total, and totalToday on wsGetOrders', () => {
        ...
        const state = orderHistorySlice(initialState, wsGetOrders(ordersData));
    ...
    it('should do nothing on wsConnecting', () => {
        const state = orderHistorySlice(initialState, wsConnecting());
        expect(state).toEqual(initialState); 
    });

    it('should set orders, total, and totalToday on wsHistoryOnMessage if success is true', () => {
        const messageData = {
            success: true,
            orders: [{
        ...
        const state = orderHistorySlice(initialState, wsHistoryOnMessage(messageData));
    ...
    it('should not update orders, total, and totalToday on wsHistoryOnMessage if success is false', () => {
        const messageData = {
            success: false,
            orders: [{
